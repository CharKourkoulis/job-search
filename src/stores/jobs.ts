...
export const useJobsStore = defineStore('jobs', {
  ...
  getters: {
    [UNIQUE_ORGANIZATIONS](state) {
      const unique_organizations = new Set<string>()
      state.jobs.forEach((job) => {
        unique_organizations.add(job.organization)
      })
    ...
    [INCLUDE_JOB_BY_SKILL]: () => (job: Job) => {
      const userStore = useUserStore()
      return job.title.toLowerCase().includes(userStore.skillsSearchTerm.toLocaleLowerCase())
    },
